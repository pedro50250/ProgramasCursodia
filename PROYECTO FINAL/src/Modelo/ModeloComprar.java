...
public class ModeloComprar {
			...
			if (cve_art == art.getCve_art()) {
				...
				if (existe) {
					...
					this.articulosSelec.set(indice, new articulo(art.getCve_art(), art.getNom_art(),
							art.getPre_art(),art.getCat_art(), art.getIdprov_art(), cantNueva));
				}
				...
				//al arraylist de articulos seleccionados
					...
					this.articulosSelec.add(new articulo(art.getCve_art(), art.getNom_art(),
							art.getPre_art(),art.getCat_art(), art.getIdprov_art(),  cantArt));
				}
	...
	@SuppressWarnings("unused")
	public boolean DiferenteProveedor() {
		int dif=0;
		boolean diferente=false;
		int prov= articulosSelec.get(0).getIdprov_art();
		for(articulo a: articulosSelec) {
			
			//System.out.println("Proveedor: "+a.getIdprov_art());
			if(prov != a.getIdprov_art()) {
				diferente = true;
			}
		}
		//System.out.println(dif + " Proveedores diferentes");
		return diferente;
	}
